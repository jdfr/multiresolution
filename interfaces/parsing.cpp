...
po::options_description globalOptionsGenerator() {
    ...
    opts.add_options()
        ("save-contours", "If this option is specified, the processed and raw contours will be provided as output (in addition to the toolpaths)")
        ...
        ("motion-planner", "If this option is specified, a very simple motion planner will be used to order the toolpaths (in a greedy way, and without any optimization to select circular contour entry points)")
        ("subtractive-box-mode", po::value<std::vector<int>>()->multitoken()->value_name(""), "If specified, it takes two numbers: LIMIT_X and LIMIT_Y, which are the semi-lengths in X and Y of a box centered on the origin of coordinates. Toolpaths will be generated in the box, EXCEPT for the input mesh file. This can be used as a crude way to generate a shape in a subtractive process. If the input mesh file is not contained within the limits, results are undefined.")
        ("slicing-uniform,u", po::value<double>(), "The input mesh will be sliced uniformly at the specified slicing step. All processed will be applied to every slice. Slices will be processed independently. Should not be used for true multislicing.")
        ("slicing-scheduler,s", po::value<std::vector<int>>()->multitoken()->zero_tokens(), "Slices for each process will be scheduled according to the Z resolution of each process. Slices of lower-resolution processes will be taken into account for slices of higher-resolution processes. If no values are provided, all specified processes are used in the multislicing process. Otherwise, the values are the indexes of the processes to be used (so that some processes can be specified but not actually used), starting from 0.")
        ("slicing-manual,m", po::value<std::vector<double>>()->multitoken(), "Same as slicing-scheduler, but the executing order is specified manually: values are Z_1, NTOOL_1, Z_2, NTOOL_2, Z_3 NTOOL_3, ..., such that for each the i-th scheduled slice is at height Z_i, and is computed with process NTOOL_i.")
        ("vertical-correction,j", "If specified, the algorithm takes care to avoid toolpaths with big voxels if the object is too thin in Z (only relevant for slicing-scheduler or slicing-manual)")
        ("z-epsilon,l", po::value<double>()->default_value(1e-6), "For slicing-scheduler or slicing-manual, Z values are considered to be the same if they differ less than this, in the mesh file units")
        ("addsub,k", "If not specified, the engine considers all processes to be of the same type (i.e., all are either additive or subtractive). If specified, the engine operates in add/sub mode: the first process is considered additive, and all subsequent processes are subtractive (or vice versa)")
        ;
    return opts;
}
...
po::options_description perProcessOptionsGenerator() {
    ...
    opts.add_options()
        ("process,p", po::value<int>()->required(), "Multiple fabrication processes can be specified, each one with a series of parameters. Each process is identified by a number, starting from 0, without gaps (i.e., if processes with identifiers 0 and 2 are defined, process 1 should also be specified). Processes should be ordered by resolution, so higher-resolution processes should have bigger identifiers. All metric parameters below are specified in mesh units x 1000 (so, if mesh units are millimeters, these are specified in micrometers. See below for an example")
        ("radx,x", po::value<double>()->required(), "radius of the voxel for the current process in the XY plane")
        ("voxel-profile,v", po::value<std::string>(), "required if slicing-scheduler or slicing-manual are specified: the voxel profile can be either 'constant' or 'ellipsoid'")
        ("voxel-z,z", po::value<std::vector<double>>()->multitoken(), "required if slicing-scheduler or slicing-manual are specified: the first value is the voxel radius in Z. The second value is the semiheight in Z (used to the define the slicing step for slicing-scheduler). If the second value is not present, it is implied to be the same as the first value.")
        ("gridstep,g", po::value<double>()->required(), "grid step for the current process (this is the minimal amount the head can be moved in XY)")
        ...
        ("smoothing,o", po::value<double>(), "If snap is not specified and clearance is not specified for the current process, this MUST be specified, and it is the smoothing radius for the computed contours")
        ("tolerances,t", po::value<std::vector<double>>()->multitoken(), "Values of the roundness parameters for the current process. The first value is for the XY radius scale, the second for the gridstep scale (if the second value is omitted, it is copied from the first one).")
        ("radius-removecommon,e", po::value<double>()->default_value(0.0), "If a positive value is specified, contours are clipped in zones where already-computed low-res contours are nearer than this value")
        ("medialaxis-radius,a", po::value<std::vector<double>>()->multitoken(), "If specified, it is a series of factors in the range 0.0-1.0. The following algorithm is applied for each factor: toolpaths following the medial axis of the contours are generated in regions of the raw contours that are not covered by the processed contours, in order to minimize such non-covered regions. The lower the factor, the more likely the algorithm is to add a toolpath.")
        ("infill,i", po::value<std::string>(), "If specified, the value must be either 'lines' (infilling is done with lines), 'concentric', (infilling is done with concentric toolpaths), or 'justcontour' (this is useful for the shared-library use case: infillings will be generated outside the engine; the engine just provides the contours to be infilled)")
        ...
        ("infill-medialaxis-radius,d", po::value<std::vector<double>>()->multitoken(), "Same as medialaxis-radius, but applied to regions not covered by infillings inside processed contours, if infill and infill-recursive are specified")
        ;
    return opts;
}
